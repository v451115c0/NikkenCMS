        ...
        try {
            if(trim($PersonType) == 'FISICA'){
                ...
                if(trim($formato) === 'pdf'){
                    ...
                    ConvertApi::setApiSecret('2BkURXcVE7djyppx');
            ...
            $conexion = \DB::connection('mysqlTV');
                    $response = $conexion->update("UPDATE users_fiscal_files SET processed = 1, last_error_message = 'pospuesto' WHERE sap_code = $sap_code");
                \DB::disconnect('mysqlTV');
            $logExec = "[" . date('Y-m-d H:i:s') . "] pospuesto: $sap_code\t";
            Storage::append("logValidaPDFFiscal.txt", $logExec);
        } finally {
            $conexion = \DB::connection('mysqlTV');
                    $response = $conexion->update("UPDATE users_fiscal_files SET processed = 1, last_error_message = 'pospuesto' WHERE sap_code = $sap_code");
